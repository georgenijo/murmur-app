#!/usr/bin/env python3
"""
Test all Deepgram API model weights.

Usage:
    python tests/test_deepgram.py <audio_file> --reference "the actual words spoken"
    python tests/test_deepgram.py test_audio.wav --reference "hello world this is a test"

Note: Requires DEEPGRAM_API_KEY in .env file.
"""
...
def test_model(model_name: str, audio_path: str, reference: str = None) -> dict:
    ...
    print(f"\n{'='*60}")
    print(f"Testing Deepgram: {model_name}")
    print('='*60)
...
def main(audio_path: str = None, reference: str = None):
    parser = argparse.ArgumentParser(description="Test Deepgram models")
    ...
    if not audio_path:
        print("Usage: python tests/test_deepgram.py <audio_file> --reference \"transcript\"")
        print()
        print("Example:")
        print("  python tests/test_deepgram.py test_audio.wav --reference \"hello world\"")
        print()
        print(f"Models tested: {', '.join(MODELS)}")
        print()
        print("Note: Requires DEEPGRAM_API_KEY in .env file")
        sys.exit(1)
    ...
    print(f"Testing {len(MODELS)} Deepgram models...")
    ...
    print("\n")
    print("="*80)
    print("DEEPGRAM MODEL COMPARISON")
    print("="*80)

    if reference:
        print(f"\n{'Model':<12} {'Transcribe':<12} {'Total':<10} {'Accuracy':<12} {'WER':<10}")
    else:
        print(f"\n{'Model':<12} {'Transcribe':<12} {'Total':<10} {'Status'}")
    print("-"*80)
    ...
    for r in results:
        if r["error"]:
            ...
            print(f"{r['model']:<12} {r['transcribe_time']:.2f}s        {r['total_time']:.2f}s      {r['accuracy']['accuracy']:.1f}%        {r['accuracy']['wer']:.1%}")
        else:
            print(f"{r['model']:<12} {r['transcribe_time']:.2f}s        {r['total_time']:.2f}s      OK")
