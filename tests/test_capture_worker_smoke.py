    ...
    def test_smoke_exercises_hello_and_start(self) -> None:
        fake_worker = textwrap.dedent(
            ...
            def read_frame():
                ...
                assert (magic, version, kind, reserved, actual_id, actual_nonce) == (b"MRMR", 10, 0, 0, capture_id, nonce)
                return json.loads(sys.stdin.buffer.read(length))

            def write_frame(message):
                body = json.dumps(message, separators=(",", ":")).encode()
                sys.stdout.buffer.write(struct.pack("<4sHBBIQ16s", b"MRMR", 10, 0, 0, len(body), capture_id, nonce) + body)
