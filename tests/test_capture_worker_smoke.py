...
class CaptureWorkerSmokeTests(unittest.TestCase):
    ...
    def test_v6_protocol_version_fails_closed(self) -> None:
        capture_id = 42
        nonce = bytes(range(16))
        encoded = bytearray(
            encode_control_frame(capture_id, nonce, {"type": "helloAck"})
        )
        encoded[4:6] = struct.pack("<H", 6)
        read_fd, write_fd = os.pipe()
        try:
            os.write(write_fd, encoded)
        finally:
    ...
    def test_smoke_exercises_hello_and_start(self) -> None:
        fake_worker = textwrap.dedent(
            ...
            assert sys.argv[1] == "--production-v7"
            ...
            def read_frame():
                ...
                assert (magic, version, kind, reserved, actual_id, actual_nonce) == (b"MRMR", 7, 0, 0, capture_id, nonce)
                return json.loads(sys.stdin.buffer.read(length))

            def write_frame(message):
                body = json.dumps(message, separators=(",", ":")).encode()
                sys.stdout.buffer.write(struct.pack("<4sHBBIQ16s", b"MRMR", 7, 0, 0, len(body), capture_id, nonce) + body)
