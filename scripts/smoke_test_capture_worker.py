#!/usr/bin/env python3
"""Exercise the final signed capture worker's production-v6 startup protocol."""
...
def smoke_test(worker: Path, timeout_seconds: float = 5.0) -> None:
    ...
    process = subprocess.Popen(
        [str(worker), "--production-v6", str(capture_id), nonce.hex()],
...
def main() -> int:
    ...
    if arguments.meeting:
        print("signed capture worker production-v6 meeting smoke passed")
    else:
        print("signed capture worker production-v6 startup smoke passed")
    return 0
