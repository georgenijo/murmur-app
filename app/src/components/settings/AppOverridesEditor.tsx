          ...
          {profiles.map((profile) => {
            ...
            return (
              <li key={profile.bundleId} className="space-y-3 rounded-xl border border-outline-variant/25 bg-surface-container-lowest p-3 shadow-sm">
                ...
                <div className="flex items-start justify-between gap-3 rounded-lg border border-outline-variant/30 bg-surface-container-low p-3">
                  <div>
                    <h4 className="text-xs font-medium text-on-surface">Exclude from Voice Query context</h4>
                    <p className="mt-1 text-xs text-on-surface-variant">Questions still work, but Murmur will not include this app's name, window title, or selected text.</p>
                  </div>
                  <button
                    type="button"
                    role="switch"
                    aria-checked={profile.queryContextExcluded}
                    aria-label={`Exclude ${appLabel} from Voice Query context`}
