...
export const FooterStats = memo(function FooterStats({ statsVersion }: FooterStatsProps) {
  ...
  return (
    <footer className="relative flex h-8 shrink-0 items-center gap-1.5 border-t border-[var(--ui-hairline)] bg-background/95 px-3.5 text-xs text-on-surface-variant">
      ...
      <div ref={shellRef} className="relative ml-auto">
        <button
          ...
          aria-expanded={open}
          className="dialog-eyebrow inline-flex items-center gap-1 rounded-[var(--ui-radius-control)] px-1.5 py-1 text-on-surface-variant transition-colors hover:bg-surface-container-low hover:text-on-surface focus:outline-none focus-visible:ring-2 focus-visible:ring-primary"
        >
          Insights
        ...
        {open && (
          <div className="dialog-popover absolute bottom-9 right-0 z-40 w-[min(420px,calc(100vw-32px))] p-1">
            <div className="flex items-center justify-between px-3 pt-2">
              <p className="dialog-eyebrow text-on-surface-variant">Usage insights</p>
              <button type="button" onClick={() => setOpen(false)} aria-label="Close insights" className="rounded-[var(--ui-radius-control)] p-1 text-on-surface-variant hover:bg-surface-container hover:text-on-surface">×</button>
            </div>
