...
Benchmark installed models against bundled fixtures on your own machine. Three accuracy tiers per model/fixture — raw decoder WER, normalized WER (formatting differences don't count as errors), and delivered WER after the production transform pipeline. Stress fixtures (jargon, numbers, disfluent, 64s extra-long, fast speech) desaturate ranking. Shared Metal/ANE init is measured separately rather than charged to the first model loaded. A separate five-cycle microphone startup diagnostic exercises the signed production capture path, reports whole-cycle and per-backend first-PCM distributions, fallback and exact failed setup steps, and preserves single-owner teardown and the existing backend memo. Reports export as privacy-safe JSON with optional auto-save for complete runs.
...
| Area | Location |
|------|----------|
| Rust backend | `app/src-tauri/src/` — 211 registered commands |
