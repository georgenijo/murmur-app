...
| Module | Purpose |
|--------|---------|
| `lib.rs` | App wiring: module declarations, `State`, `MutexExt`, 122 registered commands, setup, tray, run loop |
...
122 commands are registered in `lib.rs`. See [reference/commands.md](reference/commands.md) for the full signature-level list, grouped by module.
