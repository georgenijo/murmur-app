...
| Module | Purpose |
|--------|---------|
| `lib.rs` | App wiring: module declarations, `State`, `MutexExt`, 134 registered commands, setup, tray, run loop |
...
134 commands are registered in `lib.rs`. See [reference/commands.md](reference/commands.md) for the full signature-level list, grouped by module.
