...
The identity probe uses the same bounded accessibility warm-up ladder as selected-text capture: up to three attempts, each capped, separated by short gaps. It runs only when Accessibility permission is granted, and only a failed accessibility query is retried. A truthful "nothing matching is selected" answer, an application with no focused element, an unverified application identity, a probe error, and an exhausted ladder all resolve immediately to copy-only without spending the ladder. Replacement is therefore reliable in Cocoa applications. Known Chromium browsers get a single attempt, because selected-text capture reaches them through the clipboard fallback that correction deliberately never uses, so replacement can degrade to copy-only in Chromium and Electron targets.

Approved corrections update the process-memory last-delivery slot, so another correction builds on the approved text. Starting a correction while an approved review is still lingering for its Undo window supersedes that review, exactly as pressing the transform key does, so chained corrections are never refused. Undo restores that slot only when it still belongs to the same recording and correction. The raw and delivered history records stay unchanged. **Remember this correction…** opens a separate teaching review, including the exact phrase pair and available global, app, or project scopes from the frozen recording context. Only **Remember correction** persists a rule. Copying, approving, retrying, or cancelling does not teach automatically.
...
Rust unit tests cover literal spelling, exact reconstruction, ambiguous matches, malformed edits, shortcut key-repeat handling, identity-probe classification, lingering-pass supersession, and the shortcut arbitration that swallows a press while a transform or correction pass is already starting, thinking, or awaiting review. An ignored `installed_model_proposes_real_corrections` test runs the actual installed local model and production correction parser against synthetic examples. Setting `MURMUR_CORRECTION_WAV_DIR` to a directory containing `dictation.wav` and `instruction.wav` also exercises real Core ML recognition before local correction. The audio example says “Ship the release on Friday” and “Change Friday to Monday.” Frontend tests cover explicit teaching confirmation and pass-scoped review actions.
