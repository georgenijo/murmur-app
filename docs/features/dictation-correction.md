...
The identity probe uses the same bounded accessibility warm-up ladder as selected-text capture: up to three attempts, each capped, separated by short gaps. Only a failed accessibility query is retried. A truthful "nothing matching is selected" answer, an unverified application identity, a probe error, and an exhausted ladder all resolve immediately to copy-only. Replacement is therefore reliable in Cocoa applications. Known Chromium browsers get a single attempt, because selected-text capture reaches them through the clipboard fallback that correction deliberately never uses, so replacement can degrade to copy-only in Chromium and Electron targets.
