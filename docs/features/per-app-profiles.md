...
Live dictation does not retry this start sample or adopt a later application. An unavailable, partial, changing, or self-owned sample resolves an unmatched global-only context; app-specific IDE/context reads remain disabled and auto-paste later fails closed. The older bounded native/System Events compatibility detector remains available to non-delivery callers, but it cannot choose a live recording's profile or paste destination. Literal AppleScript `missing value` is normalized to unavailable rather than treated as a bundle identifier.
