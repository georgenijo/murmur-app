...
2. Host starts the instruction microphone asynchronously while it freezes an AX selection snapshot (secure fields refused). The review popover shows **connecting** until the exact audio owner reports Ready, then—and only then—shows **listening**. A release before readiness fails truthfully and keeps the frozen selection available for Retry.
...
Chromium/Electron webviews (Brave, Chrome, Slack, …) often expose no `AXSelectedText` even with a live visible selection. When the AX capture returns `NoSelection` (secure-field checks passed benignly) **or `AxUnavailable`** (AX failed somewhere *other than* the secure-field check — e.g. the focused-element query failed/timed out; Chromium routinely misses the 25ms messaging deadline), known Chromium **browser** bundle IDs make one secure-field-aware AX attempt and then go directly to fallback; native, Electron, and unknown apps retain the three-attempt/250ms AX warm-up ladder. This avoids two browser retry gaps observed in shipped telemetry without weakening the fail-closed check. The fallback snapshots the **full pasteboard** (every item, every type — images/files survive, #335) → overwrites with a unique text sentinel → records the pasteboard `changeCount` → sends synthetic Cmd+C → polls until the pasteboard shows **exactly one** ownership change past the sentinel write with non-sentinel text (300ms deadline; with nothing selected Cmd+C is a no-op and the fallback times out; a count that moves more than one step means a third-party writer — clipboard manager, Universal Clipboard — interleaved, and that text is rejected, #335) → restores the user's pasteboard in full, success or failure. The fallback never runs for a positively detected `SecureField`, for `AccessibilityDenied`, or for an **errored secure-field check** (`secure_check_failed`, #334 — if the AXSubrole/AXRole read failed or timed out we could not prove the focused element is not a password field, so no synthetic Cmd+C; the bar is **sticky for the whole capture**). Falling back on the remaining `AxUnavailable` cases is safe because the fallback only reproduces the user's own Cmd+C gesture and reads nothing via AX: secure fields refuse Copy system-wide, so against a secure field the sentinel never changes and the fallback times out — it can fail, never leak. If the fallback fails, the AX path's terminal error is preserved (#336): `ax_unavailable` shows "Couldn't read the selection", and `no_selection` ("Select some text first") is reserved for the case where AX itself reported an empty selection. Residual limitation of the `changeCount` guard: it counts pasteboard changes but cannot attribute a writer, so if the synthetic Cmd+C is swallowed (the Cmd+Opt+C browser case) and **exactly one** foreign write lands inside the 300ms window, that single write is indistinguishable from a real copy and is accepted. Remaining limitation: the snapshot has no AX range/bounds, so the popover centers and apply uses the paste fallback. Logging stays content-free (`via="clipboard_fallback"` + length bucket only).
...
- Arming the transform key prewarms the signed helper/model concurrently with selection capture and instruction recording. The final request still owns the exact pass and is the only path that can publish a proposal.
- Protocol v3 streams bounded, contiguous output chunks to the host. The host emits a cumulative preview only to the `transform-review` webview, never to the broadcast state event or structured telemetry; preview text is not stored in the apply session and Approve remains disabled until the final `Result` matches the streamed output and the pass reaches Ready.
