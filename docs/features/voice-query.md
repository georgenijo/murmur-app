...
Configuration limits are 32 fixed arguments, 4 KiB per argument, 32 KiB total fixed arguments, a 32 KiB spoken question, 256-byte app name, 1 KiB window title, 8 KiB selected-text context, a 256 KiB answer, and a 5–300 second timeout. Question and context are framed as untrusted reference data inside exactly one final literal argv element; no shell is involved. Stdout is decoded incrementally across split UTF-8 sequences. Missing/non-executable paths, non-zero exits, timeouts, oversized output, and empty output surface stable actionable errors without paths or content in telemetry.
