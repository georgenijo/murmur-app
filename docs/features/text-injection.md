...
1. Check `AXIsProcessTrusted()` — if accessibility is not granted, write the transcript to the clipboard and stop.
2. Wait asynchronously for the configurable delay (default 0ms) before main-thread dispatch, so macOS activation and Space notifications can advance while focus settles.
3. Re-check that the exact `recording_id` still owns delivery. A stale completion stops without writing or pasting, so it cannot overwrite a newer recording's delivery state.
