...
| `set_overlay_surface` | `expanded: bool`, `preview_visible: bool` | `Result<AppliedSurface, String>` | Composes the below-notch preview row and hover-dropdown height independently and resizes the overlay window, returning the applied frame `{windowW, windowH}` as a resize acknowledgment. The frontend's expansion controller awaits this before revealing the dropdown, so CSS never animates into a window that has not yet grown. |
