...
| `set_transform_popover_expanded` | `expanded: bool` | `Result<(), String>` | Resizes/repositions the popover to the `expanded` (ready/failed) or `compact` (listening/thinking) box, against the anchor cached by the last `show_transform_popover` call. Not part of the PR-C1 issue's literal command list — added so Rust stays the sole author of every popover pixel across state transitions, mirroring `set_overlay_expanded`. |
