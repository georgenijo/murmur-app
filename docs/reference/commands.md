...
| `get_overlay_geometry` | _(none)_ | `OverlayGeometry` | Returns the current overlay geometry contract (window/pill/dropdown/preview-row dimensions), derived from the cached notch via `geometry_for()`. Never null — a synthetic fallback notch is substituted when none is detected. |
| `set_overlay_surface` | `expanded: bool`, `preview_visible: bool` | `Result<AppliedSurface, String>` | Composes the below-notch preview row and hover-dropdown height independently and resizes the overlay window, returning the applied frame `{window_w, window_h}` as a resize acknowledgment. The frontend's expansion controller awaits this before revealing the dropdown, so CSS never animates into a window that has not yet grown. |
