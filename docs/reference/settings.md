...
| `smartAutoMicrophoneEnabled` | `boolean` | `false` | `true` / `false` | Opt-in capture routing. Requires an approved input with a successful signal check from the past 120 seconds. The next recording resolves once from the current bounded inventory and freezes that stable ID for the generation. Manual microphone selection disables it and does not require Auto evidence. |
