...
| `queryProvider` | `QueryProviderId` | `'custom'` | `'claude'`, `'codex'`, `'grok'`, `'cursor'`, or `'custom'` | Chooses discovery/auth metadata, recommended argv, and the post-spawn output adapter. Claude recommends stream-json and Codex recommends `exec --json`; every provider still uses the generic direct-spawn bridge. Unknown persisted values fail closed to Custom. |
