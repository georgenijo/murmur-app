...
| `smartAutoMicrophoneEnabled` | `boolean` | `false` | `true` / `false` | Opt-in capture routing among approved, available inputs. With background checks off, recording needs no prior signal check. With background checks on, the input needs a successful check from the past 120 seconds. The next recording resolves once from the current bounded inventory and freezes its policy and stable ID for the generation. Manual microphone selection disables Auto. |
| `smartAutoProbeEnabled` | `boolean` | `false` | `true` / `false` | Separate consent for bounded idle-only signal checks of approved inputs. Existing and malformed settings default false. Requires Auto routing, no pinned input, and microphone permission already granted. Failed rounds back off and park after three rounds. No transcription, saved audio, or per-device probe record. |
...
| `smartAutoPreferredDeviceIds` | `string[]` | `[]` | ordered subset of approved IDs | The current verified input is retained. When another input is needed, this order ranks freshly verified eligible candidates before the approved macOS default and deterministic external fallback. |
