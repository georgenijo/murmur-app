...
| `delivery-retry-feedback` | `{kind, message}`, where `kind` is `"auto_pasted"` \| `"clipboard_only"` \| `"empty"` \| `"busy"` \| `"failed"` | `delivery_recovery.rs` | Each `retry_last_delivery` attempt publishes its bounded result, including no prior delivery and concurrent retry refusals. | Main banner (`useDeliveryRecoveryListeners`). |
