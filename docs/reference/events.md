...
| `audio-recovery-started` | `{recordingId, reason}` | `commands/recording.rs` | A starting/recording owner was cancelled and detached. The app briefly reports `recovering`, releases logical ownership, and returns to `idle` without waiting for its worker to exit. | Overlay recovering cue and diagnostics. |
