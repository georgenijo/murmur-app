...
| `audio-input-inventory-changed` | `AudioInputInventorySnapshot {schemaVersion, revision, status, devices, defaultInputId, errorCode}` | `audio_inventory.rs` | Targeted to the main window after a passive-worker invalidation, startup refresh, or five-minute fallback refresh materially changes topology, default input, availability, or error state. Unchanged refreshes emit nothing; a refresh requested during capture is coalesced until joined-worker `Idle`. Labels and stable IDs remain local. | `useAudioInputInventory`; Settings, settings migration, and the corpus recorder consume its validated snapshot. |
