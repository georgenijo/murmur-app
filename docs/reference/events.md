...
| `smart-auto-microphone-changed` | `null` | `audio_inventory.rs` | Content-free broadcast after signal evidence changes, capture selection, capture failure, sleep, wake, or an inventory event. Consumers reread the cached decision. No device IDs, labels, or audio enter this event. | Settings and overlay (`useSmartAutoMicrophoneStatus`). |
