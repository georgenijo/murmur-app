...
| `audio-input-inventory-changed` | `AudioInputInventorySnapshot {schemaVersion: 2, revision, status, devices, defaultInputId, lidState, errorCode}` | `audio_inventory.rs` | Targeted to the main and overlay windows after a passive-worker invalidation, startup refresh, or five-minute fallback refresh materially changes topology, default input, availability, or error state. Device metadata records native transport, connectivity, and input capability; it never derives a class from a label. Unchanged refreshes emit nothing; a refresh requested during capture is coalesced until joined-worker `Idle`. Labels and stable IDs remain local. | `useAudioInputInventory`; Settings, overlay, settings migration, and the corpus recorder consume its validated snapshot. |
...
| `smart-auto-microphone-changed` | `null` | `audio_inventory.rs`, `smart_auto_probe.rs` | Content-free broadcast after evidence, selection, failure, sleep/wake, inventory, probe-policy, or probe-phase changes. Consumers reread the cached decision. No device IDs, labels, or audio enter this event. | Settings and overlay (`useSmartAutoMicrophoneStatus`). |
...
| `dictation-partial` | `{recordingId, text}` | `commands/recording.rs` | A provisional Core ML snapshot, targeted only to the `dictation-preview` window. Stale generations are rejected before and after decoding; the text never enters delivery, history, stats, files, or telemetry. | Live dictation preview popover (`useDictationPartial`) while the exact generation is recording. |
...
| `transcription-complete` | `{recordingId, text, rawText, duration, teachingContext, interrupted, recording: {recordingId, modelId, modeId, profileId, stages: [{stage, outcome, changed}]}}` | `commands/recording.rs` | After a non-empty transcription is delivered. `rawText` is the backend recognition before transforms. `interrupted` is null or `{reason, deliveredSamples, durationMs}`. `teachingContext` is nullable, and Mode and profile IDs in the immutable recording snapshot are nullable. Broadcast to all windows. | Main window (`useRecordingState` → history, stats, display). |
...
| `auto-paste-failed` | `string` (hint) | `commands/recording.rs` via `injector.rs` | Clipboard or automatic-paste delivery failed, a focus safety check refused paste, or completion could not be confirmed. Messages claim clipboard readiness only after a confirmed write. Disabled auto-paste and missing Accessibility permission do not emit this error event. | Main window (`useRecordingState`, detailed hint with inline retry for 5s). |
| `dictation-delivery-outcome` | `{recordingId, outcome}` where `outcome` is `"clipboardOnly"` \| `"autoPastePosted"` \| `"clipboardWriteFailed"` \| `"unconfirmed"` | `commands/recording.rs` | Once after the current recording's text-delivery attempt. Content-free and generation-gated; stale/cancelled recordings do not emit. | Overlay (`useOverlayRuntime`): shows the bounded manual-paste cue only for `clipboardOnly`, and newer outcomes clear older cues. |
...
| `correction-start-failed` | `string` | `correction_shortcut.rs` | A correction shortcut could not start or finish its exact transform pass. Targeted to `main`; the main window is then shown. | Main banner (`useDeliveryRecoveryListeners`). |
| `delivery-retry-feedback` | `{kind, message}`, where `kind` is `"auto_pasted"` \| `"clipboard_only"` \| `"empty"` \| `"busy"` \| `"failed"` | `delivery_recovery.rs` | Each `retry_last_delivery` attempt publishes its bounded result, including no prior delivery and concurrent retry refusals. | Main banner (`useDeliveryRecoveryListeners`). The initiating overlay uses its command's returned result so unrelated or stale broadcasts cannot revive a cue. |
| `corpus-recording-status` | `{state, error}`, where `state` is `"starting"` \| `"recording"` \| `"saving"` \| `"recovering"` \| `"idle"` \| `"error"` | `commands/corpus.rs` | Each personal-corpus capture transition. `error` is null outside the error state. | Settings corpus recorder. |
...
| `meeting-speakers-updated` | `{sessionId}` | `meeting_diarization.rs` | The current idle-only speaker pass commits conservative remote-speaker assignments for the exact meeting session. Contains no transcript, speaker name, or turn data. | Main (`useMeetings` refreshes meeting detail). |
...
| `meeting-suggestion-changed` | `{token, title, startMs, endMs} \| null` | `meeting_suggestions.rs` | A consented Calendar suggestion appears, expires, is invalidated, or is dismissed. Sent only to main and overlay; contains a private event title and must never be logged. | Main and overlay suggestion hooks. |
| `accept-meeting-suggestion` | `{token}` | Overlay frontend, targeted to main | Explicit Accept click. Carries no title or attendee override and starts no capture itself. | Main routes the token through `useMeetings.start` to normal `start_meeting` admission. |
| `meeting-audio-invalidated` | `{sessionId: string \| null}` | `meeting_audio.rs`, called by meeting deletion and pruning | Before retained audio for the selected session may be removed. Pruning, including meeting-start retention, emits only for sessions selected for removal; no-op pruning emits nothing. A null session ID is reserved for Delete all. Pending byte reads are also revoked. Sent only to main. | Main playback hook stops sources and discards stale reads and decodes. |
...
Transform events carry a `transformPassId` where a pass exists, so a delayed handler can prove whether it still owns the flow. State and lifecycle events carry no instruction, selection, or proposal text. The targeted `transform-proposal-chunk` event carries proposal text only to the transform-review window.
...
| `transform-capture-starting` | `null` | `transform_flow.rs` | Accepted transform, correction, or retry immediately before microphone arm, including the interval before AX selection completes. Targeted to the main window; carries no content. | Meeting suggestion coordinator and main retained-audio playback hook. |
...
| `transform-proposal-chunk` | `{transformPassId, sequence, text}` | `transform_flow.rs` | A current Thinking pass receives a proposal chunk. Targeted only to `transform-review`; sequence zero expands the popover before delivery. | Transform popover (`useTransformReviewDriver`). |
...
Question and context content never appears in events. Only the dedicated review webview receives answer chunks and listening partials; it pulls the context summary through the gated review-content command.
...
| `assistant-conversation-changed` | `{conversationId, queryPassId}` | `assistant.rs` | Native queue, partial answer, or terminal state changed. Fetch the conversation by ID; no content in the notification. | Main only, targeted. |
| `assistant-state-changed` | `{conversationId, queryPassId, state, errorCode}` | `query_flow.rs` | Exact thread and pass lifecycle, separate from the content-free global query state. | Main only, targeted. |
| `assistant-partial` | `{conversationId, queryPassId, text}` | `query_flow.rs` | Ephemeral listening transcription for an Assistant voice turn; never saved or sent to Pi. | Main only, targeted. |
| `assistant-draft-state` | `{conversationId, queryPassId, state, errorCode}` | `query_flow.rs` | Local composer capture lifecycle. Successful finish emits idle before returning final text; idle with cancelled error discards speech. No Pi request. | Main only, targeted. |
| `assistant-draft-partial` | `{conversationId, queryPassId, text}` | `query_flow.rs` | Ephemeral local ASR preview for the composer. Exact-pass gated; never stored or broadcast to the floating transcript. | Main only, targeted. |
| `assistant-open-conversation` | `{conversationId}` | `query_flow.rs` | Navigate main to the exact imported Ready popover exchange after explicit persistence consent. | Main only, targeted. |
...
| `query-review-hidden` | `{queryPassId}` | `query_flow.rs` | Exact-pass cancellation/close completes and the popover is hidden. The main window folds an otherwise-unreported cancellation into content-free usage statistics exactly once; malformed and stale payloads cannot clear a newer pass. | Main window and query popover. |
...
| `overlay-visible-changed` | `boolean` | `commands/overlay.rs` | `show_overlay` emits `true` after showing the window; `hide_overlay` emits `false` after hiding it. | Overlay: gates the expansion controller's cursor poller. Defaults to visible on mount so first hover works before an event. |
...
| `diagnostics-tab-requested` | one of `"events"`, `"runs"`, `"performance"`, `"latency"`, `"reports"`, or `"transforms"` | `commands/performance.rs` | `show_diagnostics_window` shows and focuses the Diagnostics window, then targets the requested validated tab to that window. | Diagnostics window (`DiagnosticsWindowApp`). |
