...
| `dictation-generation-started` | `{recordingId}` | `commands/recording.rs` | When a live or legacy in-memory dictation generation claims the pipeline. Content-free and monotonic. | Overlay (`useOverlayRuntime`): advances its stale-event floor and clears older clipboard-only cues. |
