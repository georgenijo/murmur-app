...
| `audio-recovery-started` | `{recordingId, reason}` | `commands/recording.rs` | A starting/recording owner was cancelled. The app reports `recovering` and retains exclusive logical ownership until its worker exits, then returns to `idle`. | Overlay recovering cue and diagnostics. |
| `recording-initialization-failed` | `{recordingId, error, errorKind}` | `commands/recording.rs` | Initialization/runtime capture failed or crossed the 30-second hard deadline. `error` is a fixed user-facing message and `errorKind` is a stable content-free enum; raw CPAL messages are never emitted. Emitted exactly once per generation. | Main error surface; overlay bounded failure flash. |
