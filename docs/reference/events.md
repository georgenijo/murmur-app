...
| `dictation-partial` | `{recordingId, text}` | `commands/recording.rs` | A provisional Core ML snapshot, targeted only to the overlay. Stale generations are rejected before and after decoding; the text never enters delivery, history, stats, files, or telemetry. | Overlay (`useDictationPartial`) while the exact generation is recording. |
