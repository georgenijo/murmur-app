...
| `transform-capture-starting` | `null` | `transform_flow.rs` | Accepted transform, correction, or retry immediately before microphone arm, including the interval before AX selection completes. Targeted to the main window; carries no content. | Meeting suggestion coordinator. |
