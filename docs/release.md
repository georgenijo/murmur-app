...
AUHAL request for a missing preferred device with default fallback enabled, and
requires both stream-open and completed device resolution. This catches sandbox,
entitlement, and preferred-device fallback regressions that static plist and
signature checks cannot.
