...
### Added
- **Transform selection capture now works in Chromium/Electron apps** (Brave, Chrome, Slack, …): when the webview exposes no accessible selection, capture falls back to a sentinel-guarded synthetic Cmd+C that restores the clipboard afterwards. The fallback only runs after the secure-field checks pass; password fields, denied Accessibility, and AX failures stay fail-closed (#329).
